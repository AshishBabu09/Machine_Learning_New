...
def fit_and_predict(training_data, labels, test_data, n_neighbors=3):
    knn = KNeighborsClassifier(n_neighbors=n_neighbors)
    knn.fit(training_data, labels)
    predicted_classes = knn.predict(test_data)
    return predicted_classes

def plot_test_data(test_data, predicted_classes):
...
# Fit the classifier to the training data and predict the classes for the test data
predicted_classes = fit_and_predict(training_data, labels, test_data)

# Plot the test data with predicted classes
plot_test_data(test_data, predicted_classes)
